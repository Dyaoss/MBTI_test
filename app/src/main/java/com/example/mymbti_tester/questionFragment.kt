...
class questionFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        for (i in questionTextView.indices) {
            ...
            radioButton1.text = getString(questionAnswers[questionType][i][1])
        }
        return view
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btn_next.setOnClickListener {
            val isAllAnswered =
                answerRadioGroup.all { it.checkedRadioButtonId != -1 } // 모든 항목이 선택되어있는지 -> 되어있으면 true

            if(isAllAnswered){
                val responses = answerRadioGroup.map { radioGroup ->
                    val firstRaioButton = radioGroup.getChildAt(0) as RadioButton
                    if(firstRaioButton.isChecked) 1 else 2
                }
